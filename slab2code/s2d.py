...
def main(args):
            ...
            if filename.endswith(".csv"):
                ...
                if slabname == slab:
                    ...
                    if etype == "AS":
                        ...
                        default_uncertainty = 10
                    f = NewFile(
                        args.database + "/" + filename, default_uncertainty, etype, name
                    )
                    filelist.append(f)
                else:
                    pass
                ...
                print(
                    "The file %s was not written to the dataset - FILES MUST BE IN .CSV FORMAT"
                    % filename
                )
        ...
        for filename in files:
            if filename.endswith(".csv"):
                ...
                if args.bounds is not None:
                    ...
                    latmin = args.bounds[2]
                    latmax = args.bounds[3]
                    slabname = rectangleIntersectsPolygon(
                        lonmin, lonmax, latmin, latmax
                    )
                    if slabname in slablist:
                        # Creates file object to be written to output file
                        f = NewFile(
                            args.database + "/" + filename, default_uncertainty, etype
                        )
                        filelist.append(f)
                    else:
                        pass
        ...
        for file in args.input:
            try:
                ...
                try:
                    ...
                    print(
                        "Input file information must be given as infile,unc,etype \
                        or as infile,etype"
                    )
    ...
    if len(filelist) == 0:
        print(
            "No input files were provided. Include argument: -i infile1,unc1,etype1, -i \
