    ...
    try:
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(
                "Input file information must be \
...
def writetofile(
    ...
    try:
        if input_file.endswith(".csv"):
            ...
            print(
                "Input file %s was not written to file. MUST BE IN .CSV FORMAT"
                % input_file
            )
            pass
    except:
        print(
            "Could not read file %s. A header line of column labels \
        followed by a deliminated dataset is expected. Check file format to ensure this \
        is such. All files must be in .csv format."
            % input_file
        )
    ...
    try:
        ...
        try:
            ...
            print(
                "If file contains earthquake information (event-type = EQ), \
...
def write_data(df, output_file):
    ...
    if not os.path.isfile(output_file):
        with open(output_file, "w") as f:
            df.to_csv(
                f, header=True, index=False, float_format="%0.3f", na_rep=float("nan")
            )
...
def inbounds(args, data, slab):
            ...
            try:
                ...
                try:
                    data["time"] = pd.to_datetime(
                        data["time"], format="%m-%d-%YT%H:%M:%S.%f"
                    )
                except:
                    ...
                    for index, row in data.iterrows():
                        print(row["time"])
                        try:
                            row["time"] = pd.to_datetime(
                                row["time"], format="%m-%d-%YT%H:%M:%S"
                            )
                        except:
                            try:
                                row["time"] = pd.to_datetime(
                                    row["time"], format="%m-%d-%YT%H:%M:%S.%f"
                                )
                            except:
    ...
    if args.startTime and args.endTime and args.startTime >= args.endTime:
        print(
            "End time must be greater than start time.  Your inputs: Start %s \
        End %s"
...
def moment_calc(df, args, seismo_thick, slabname):
    ...
    try:
        ...
        try:
            ...
            print(
                "Where moment tensor information is available, columns \
...
def makeframe(data, fcsv, event_type, uncertainty, args, seismo_thick, slabname):
    ...
    if "unc" in data.columns and "q" in data.columns:
        try:
            ...
            print(
                "When adding a file with uncertainty quality, the column \
...
def plot_map(
    ...
    m.etopo()
    m.drawmeridians(
        np.arange(llcrnrlon, urcrnrlon, 5), labels=[0, 0, 0, 1], fontsize=10
    )
    m.drawparallels(
        np.arange(llcrnrlat, urcrnrlat, 5), labels=[1, 0, 0, 0], fontsize=10
    )
    x, y = m(lons, lats)
...
def find_closest(eqo, eqm1, eqm2):
    ...
    print(
        "-------------------------------------- lat %s lon %s depth %s mag %s time \
        %s catlog"
        % (",", ",", ",", ",", ",")
    )
    ...
    # If all things are equal, the first event is chosen as a match by default
        ...
        print(
            "The two events are equidistant to the match in time, space, and magnitude.\
...
def removematches(dfo, dfm):
    ...
    ind = (
        dfo.time.isin(dfm.time)
        & dfo.lat.isin(dfm.lat)
        & dfo.lon.isin(dfm.lon)
        & dfo.mag.isin(dfm.mag)
...
def rid_matches(cat1, cat2, name1, name2):
    ...
    try:
        name1w = name1[
            -10:
        ]  # this doesn't make sense, and seems to chop the file name inappropriately - will have to resolve this later.
        name2w = name2[-10:]
    except:
        name1w = name1[:-4]
        name2w = name2[:-4]
    ...
    print(
        "%i matches were found between the catalogs: %s and %s." % (count, name1, name2)
    )
    if count > 0:
        with open(matchfile, "w") as f:
            matches.to_csv(f, header=True, index=False, float_format="%0.4f")
        print(
            "The pairs can be found in the file: ** %s **, which has been written added to the current directory."
            % (name1w + name2w + "-matches.csv")
        )
        print(
            "Based on the order of entry, in the instance of duplicate events, the entries in ** %s ** were added to the slab file while the entries in ** %s ** were not added."
            % (name1, name2)
        )
