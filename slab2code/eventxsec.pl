...
####
if (@ARGV < 6){print "Must input 6 string arguments: \n";
                ...
                print "    5) Slab1.0 grid or main guide \n";
                ...
                print "      9) extra grid to compare. Enter na for no extra grid \n";
                ...
                print "               default is maximum data point depth (km) \n";
		        print "	     13) file with lon, lat, depth, mag of event (separated by space) \n";
                print "      14) file with MT info \n";
    
exit
}
if (@ARGV > 14){print "Must input 6 string arguments: \n";
                print "    1) slab (e.g. alu, sam, phi) \n";
                print "    2) model date (MM.DD.YY) \n";
                print "    3) model directory name and location \n";
                print "    4) input file name and location \n";
...
if (@ARGV < 14 & @ARGV > 6){print "Must input 6 string arguments: \n";
                print "    1) slab (e.g. alu, sam, phi) \n";
                print "    2) model date (MM.DD.YY) \n";
                print "    3) model directory name and location \n";
                print "    4) input file name and location \n";
...
if (@ARGV == 14){($ID,$folder,$folderloc,$unfiltered,$tgs,$sp,$reg_trench,$caz,$guide,$xclen,$xclen2,$maxdepthplot,$eventfile,$MTfile)=@ARGV;}
if($reg_trench eq "na"){$reg_trench="library/forplotting/trenches_usgs_2017_depths.csv";}

print "$ID,$folder,$folderloc,$unfiltered,$reg_trench,$caz,$guide,$xclen,$eventfile";
...
`$gmt makecpt -Cgray -D -Fr -T$z0/$z1/10 -Z > $depcpt`;
$depcpt2="$folderloc/$ID\_slab2_xsecs_$folder/dep2.cpt";
`$gmt makecpt -Cgray -I -D -Fr -T$z10/$z00/10 -Z > $depcpt2`;
...
open(DATu,"$unfiltered");
   ...
   $n=0;
   foreach(@dtu) {
        ($ilat[$n],$ilon[$n],$idep[$n],$iunc[$n],$itype[$n],$bah,$imag[$n],$itime[$n],$iPaz[$n],$iPpl[$n],$iTaz[$n],$iTpl[$n],$iS1[$n],$iD1[$n],$iR1[$n],$iS2[$n],$iD2[$n],$iR2[$n],$imlon[$n],$imlat[$n],$imdep[$n],$iid[$n],$isrc[$n])=split(',',$_);
        chomp($isrc[$n]);
	$imag2[$n]=$imag[$n]/10;
	$imag3[$n]=$imag2[$n]*$imag2[$n]*$imag2[$n]*3;
    $imag3[$n]=$imag2[$n];
        if($ilon[$n]<0){$ilon[$n]=$ilon[$n]+360;}
        if($itype[$n]eq$EQID){print OUTEQu "$ilon[$n] $ilat[$n] $idep[$n] $idep[$n] $imag3[$n] $iid[$n]\n";}
...
$event="$folderloc/$ID\_slab2_xsecs_$folder/event.dat";
open(OUTev,">$event");
open(DATu,"$eventfile");
   @dtu=<DATu>;close(DATu);
   $n=0;
   foreach(@dtu) {
        ($evlon,$evlat,$evdep,$evmag)=split();
        chomp($evmag);
	$evmag2 = $evmag/10;
	$evmag3 = $evmag2*$evmag2*$evmag2*3;
        print "$evlon $evlat $evdep $evdep $evmag3\n";
	if($evlon<0){$evlon=$evlon+360;}
        print OUTev "$evlon $evlat $evdep $evdep $evmag3\n";
        $n++;
   }
close(OUTev);
print "event!!! $event \n";
...
foreach(@sftrlon) {
    ...
    `$gmt psxy $folderloc/$ID\_slab2_xsecs_$folder/indata/$ID\_$n\_inEQu.dat $R3 $J3 -C$depcpt2 -Sc -W0.08,grey -O -K >> $folderloc/$ID\_slab2_xsecs_$folder/$ID\_$n\_csec.ps`;

    `$gmt project $event -C$sftrlon[$n]/$sftrlat[$n] -A$sftraz[$n] $pW $pL -Q -S -Fpz > $folderloc/$ID\_slab2_xsecs_$folder/indata/$ID\_$n\_event.dat`;
    `$gmt psxy $folderloc/$ID\_slab2_xsecs_$folder/indata/$ID\_$n\_event.dat $R3 $J3 -C$depcpt2 -Sc -W0.8,red -O -K >> $folderloc/$ID\_slab2_xsecs_$folder/$ID\_$n\_csec.ps`;
    ...
    if ($MTfile ne "na"){
    `$gmt pscoupe -Ad$sftrlon[$n]/$sftrlat[$n]/$sftraz[$n]/$pL/0/$pW/0/700 $MTfile $R3 $J3 -Sc10 -Gred -O -K >> $folderloc/$ID\_slab2_xsecs_$folder/$ID\_$n\_csec.ps`;
    }
    ...
    if ($MTfile ne "na"){
    `$gmt psmeca $MTfile -R -J -Sc10 -Gred -O -K >> $folderloc/$ID\_slab2_xsecs_$folder/$ID\_$n\_csec.ps`;
    }
    `$gmt psxy $event -R -J -Sc1.5 -Gyellow -W0.05,black -O -K >> $folderloc/$ID\_slab2_xsecs_$folder/$ID\_$n\_csec.ps`;
