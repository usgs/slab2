...
def main(args):
    ...
    for folder in filelist:
        ...
        try:
            ...
            maskdf = pd.read_csv(
                "%s_clp_%s.csv" % (thisfolder, date), sep=",", names=["lon", "lat"]
            )
            slab1data = s2f.getDFinMask(slab1data, maskdf)
            eventlist = s2f.getDFinMask(eventlist, maskdf)

        eventlist = s2f.getReferenceKagan(
            slab1data, eventlist, origorcentl, origorcentd
        )
        ...
        if lendata > 0:
            ax1z.plot(
                deplist,
                normpdfD,
                label="%s, s:%.1f, d:%.1f" % (slab, taper_start, seismo_thick),
                linewidth=2,
                c=clist[n],
            )
            ax1z.plot(
                [seismo_thick, seismo_thick],
                [0.09, 0.1],
                linewidth=2,
                linestyle="dashed",
                c=clist[n],
    ...
    ax1z.set_ylabel("P")
    ax1z.set_title(
        "Slab depth distributions (surfacefilt = %i km, orig = %s, depth = %s, hist= %s)"
        % (maxdepdiff, origorcentl, origorcentd, slaborev)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="new directory to save file structure to",
    )
