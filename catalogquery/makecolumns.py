...
database = raw_input('Enter directory listing GCMT events monthly:')
oldcatalog = raw_input('Enter name of existing GCMT catalog:')
newcatalog = raw_input('Enter name to write update GCMT catalog to: (format "gcmt_MMYY.csv")')
...
for filename in files:
    ...
    for line in f:
        ...
        if i == 0:
            ...
            print 'lon,lat,depth,mag,year,month,day,hour,minute,second',linelist[4],linelist[3],linelist[5],linelist[7],year,mont,dayz,hour,minu,seco
            i += 1
...
allgcmt = pd.concat([alldat,oldgcmt])
